...
var options_schema = {
  ...
  , remove_entry        : joi.func()
  , remove_file        : joi.func()
};
    ...
    return function (files, metalsmith, done) {
        ...
        var process_file = function process_file (file) {
            ...
            json.forEach(function (element) {
                ...
                if (if options.remove_entry && options.remove_entry(data, filename)) {
                  log('Removing entry: %s', filename);
                } else {
                  files[filename] = data;
                }
            });

            if (options.remove_file && options.remove_file(file)) {
              delete files[file];
            }
        };
